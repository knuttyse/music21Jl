...
PitchType <: Pitch # TODO

PitchClassString = Union{"a", "A", "t", "T", "b", "B", "e", "E"}

STEPREF: Dict{::StepName, ::Int} = Dict(
    "C"=> 0,
    "D"=> 2,
    "E"=> 4,
    "F"=> 5,
...
STEPREF_REVERSED: dict[int, StepName] = Dict(
    0=> "C",
    2=> "D",
    4=> "E",
    5=> "F",
...
STEP_TO_DNN_OFFSET: dict[StepName, int] = Dict(
    "C"=> 0,
    "D"=> 1,
    "E"=> 2,
    "F"=> 3,
...
function _sortModifiers() begin
    for i in (4, 3, 2, 1)
        ams = []
        for (_, sym) in accidentalNameToModifier
            if length(sym) == i
                push!(ams, sym)
            end
        end
        return ams # TODO: Revisit the indentation because the loop seems to stop at 4
    end
end
...
# ------------------------------------------------------------------------------
# utility functions
function _convertPitchClassToNumber(
    ps::Union{::Int, ::Float, ::PitchClassString}
) -> ::Union{Int, Float} begin
    ps
end
...
Test.@test _convertPitchClassToNumber(3) == 3
Test.@test _convertPitchClassToNumber("a") == 10
Test.@test _convertPitchClassToNumber("B") == 11
Test.@test _convertPitchClassToNumber("3") == 3
Test.@test _convertPitchClassToNumber(3.5) == 3.5


function convertPitchClassToStr(pc: Int) -> String begin

    pc = pc % 12  # do just in case
    # replace 10 with A and 11 with B
    return "{$pc:X}"  # using hex conversion, good up to 15
end

Test.@test convertPitchClassToStr(3) == "3"
Test.@test convertPitchClassToStr(10) == "A"
Test.@test convertPitchClassToStr(11) == "B"
Test.@test convertPitchClassToStr(15) == "F"


function _convertPsToOct(ps::Union{::Int,::Float}) -> Int
    ps = round(ps, digits=PITCH_SPACE_SIG_DIGITS)
    return Int(floor(ps / 12.0)) - 1
end
Test.@test _convertPsToOct(59) == 3
Test.@test _convertPsToOct(60) == 4
Test.@test _convertPsToOct(61) == 4
Test.@test _convertPsToOct(12) == 0
Test.@test _convertPsToOct(0) == -1
Test.@test _convertPsToOct(-12) == -2
Test.@test _convertPsToOct(135) == 10
Test.@test _convertPsToOct(71.999) == 4
Test.@test _convertPsToOct(71.999999) == 4
Test.@test _convertPsToOct(72) == 5


function _convertPsToStep(ps::Union{::Int,::Float}) -> Tuple{::StepName, ::Accidental, ::Microtone, ::Int} begin
    if isa(ps, Int)
        pc = ps % 12
        alter = 0.0
        micro = 0.0
...
Test.@test _convertPsToStep(60) == ("C", Accidental(0), Microtone(0), 0)
Test.@test _convertPsToStep(66) == ("F", Accidental(1), Microtone(0), 0)
Test.@test _convertPsToStep(67) == ("F", Accidental(2), Microtone(0), 0)
Test.@test _convertPsToStep(68) == ("G", Accidental(0), Microtone(0), 0)
Test.@test _convertPsToStep(-2) == ("B", Accidental(-1), Microtone(0), 0)
Test.@test _convertPsToStep(60.5) == ("C", Accidental(0), Microtone(50), 0)
Test.@test _convertPsToStep(62.5) == ("D", Accidental(0), Microtone(50), 0)
Test.@test _convertPsToStep(135.5) == ("C", Accidental(0), Microtone(50), 1)
Test.@test _convertPsToStep(70) == ("A", Accidental(0), Microtone(0), 0)
Test.@test _convertPsToStep(70.2) == ("A", Accidental(0), Microtone(20), 0)
Test.@test _convertPsToStep(70.5) == ("A", Accidental(1), Microtone(50), 0)
Test.@test _convertPsToStep(61.5) == ("C", Accidental(1), Microtone(50), 0)
Test.@test _convertPsToStep(43.00001) == ("B", Accidental(-1), Microtone(0), 0)
Test.@test _convertPsToStep(42.999739) == ("B", Accidental(-1), Microtone(0), 0)
Test.@test _convertPsToStep(59.8) == ("B", Accidental(1), Microtone(80), 0)
Test.@test _convertPsToStep(59.9999999) == ("C", Accidental(0), Microtone(0), 0)

function _convertCentsToAlterAndCents(shift) -> Tuple{::Float, ::Float} begin
    value = Float64(shift)
    alterAdd = 0.0
    if value > 150
        increment, value = divrem(value, 100)
        alterAdd += increment
    else if value < -150
        while value < 100
            value += 100
    ...
    if value < -75
        ...
        throw(Error("value exceeded range: $value"))
    return alterShift + alterAdd, Float64(cents)
end
Test.@test _convertCentsToAlterAndCents(125) == (1.0, 25.0)
Test.@test _convertCentsToAlterAndCents(-75) == (-0.5, -25.0)
Test.@test _convertCentsToAlterAndCents(-125) == (-1.0, -25.0)
Test.@test _convertCentsToAlterAndCents(-200) == (-2.0, 0.0)
Test.@test _convertCentsToAlterAndCents(235) == (2.5, -15.0)


function _convertHarmonicToCents(value::Union{::Int,::Float}) -> Int begin
    if value < 0  # subharmonics
        value = 1 / (abs(value))
    return Int(round(1200 * math.log2(value)))
end
    
Test.@test _convertHarmonicToCents(1) == 0
Test.@test _convertHarmonicToCents(2) == 1200
Test.@test _convertHarmonicToCents(3) == 1902
Test.@test _convertHarmonicToCents(4) == 2400
Test.@test _convertHarmonicToCents(5) == 2786
Test.@test _convertHarmonicToCents(6) == 3102
Test.@test _convertHarmonicToCents(7) == 3369
Test.@test _convertHarmonicToCents(8) == 3600
Test.@test _convertHarmonicToCents(9) == 3804
Test.@test _convertHarmonicToCents(10) == 3986
Test.@test _convertHarmonicToCents(11) == 4151
Test.@test _convertHarmonicToCents(12) == 4302
Test.@test _convertHarmonicToCents(13) == 4441
Test.@test _convertHarmonicToCents(14) == 4569
Test.@test _convertHarmonicToCents(15) == 4688
Test.@test _convertHarmonicToCents(16) == 4800
Test.@test _convertHarmonicToCents(17) == 4905
Test.@test _convertHarmonicToCents(18) == 5004
Test.@test _convertHarmonicToCents(19) == 5098
Test.@test _convertHarmonicToCents(20) == 5186
Test.@test _convertHarmonicToCents(21) == 5271
Test.@test _convertHarmonicToCents(22) == 5351
Test.@test _convertHarmonicToCents(23) == 5428
Test.@test _convertHarmonicToCents(24) == 5502
Test.@test _convertHarmonicToCents(25) == 5573
Test.@test _convertHarmonicToCents(26) == 5641
Test.@test _convertHarmonicToCents(27) == 5706
Test.@test _convertHarmonicToCents(28) == 5769
Test.@test _convertHarmonicToCents(29) == 5830 
Test.@test _convertHarmonicToCents(30) == 5888
Test.@test _convertHarmonicToCents(31) == 5945
Test.@test _convertHarmonicToCents(32) == 6000

Test.@test _convertHarmonicToCents(1.0) == 0
Test.@test _convertHarmonicToCents(1/2) == -1200
Test.@test _convertHarmonicToCents(1/3) == -1902
Test.@test _convertHarmonicToCents(1/4) == -2400
...
function _dissonanceScore(pitches, smallPythagoreanRatio=true, accidentalPenalty=true, triadAward=true) begin
    ...
    if smallPythagoreanRatio
        ...
        for (p1, p2) in subsets(pitches, 2)
            ...
            try:
                ...
                penalty = (log(ratio.numerator * ratio.denominator / ratio)
...
# TODO: Tests suggested by copilot:
Test.@test _dissonanceScore([Pitch("C4"), Pitch("E4"), Pitch("G4")]) == 0.0
Test.@test _dissonanceScore([Pitch("C4"), Pitch("E4"), Pitch("G4"), Pitch("B4")]) == 0.0
Test.@test _dissonanceScore([Pitch("C4"), Pitch("E4"), Pitch("G4"), Pitch("B4"), Pitch("D5")]) == 0.0
Test.@test _dissonanceScore([Pitch("C4"), Pitch("E4"), Pitch("G4"), Pitch("B4"), Pitch("D5"), Pitch("F5")]) == 0.0

function _bruteForceEnharmonicsSearch(oldPitches, scoreFunc=_dissonanceScore)
    all_possible_pitches = [[p] + p.getAllCommonEnharmonics() for p in oldPitches[2:end]]
    all_pitch_combinations = Itertools.product(all_possible_pitches...)
    newPitches = minimum(all_pitch_combinations, by=x -> scoreFunc(oldPitches[:1] + x))
    return oldPitches[:1] + newPitches
end
# TODO: tests

function _greedyEnharmonicsSearch(oldPitches, scoreFunc=_dissonanceScore) begin
    newPitches = oldPitches[:1]
    for oldPitch in oldPitches[2:end]
        candidates = [oldPitch] + oldPitch.getAllCommonEnharmonics()
        newPitch = minimum(candidates, by=x -> scoreFunc(newPitches + [x]))
        push!(newPitches, newPitch)
    end
    return newPitches
end

function simplifyMultipleEnharmonics(pitches, criterion=_dissonanceScore, keyContext=None)
...
Test.@test simplifyMultipleEnharmonics([11, 3, 6]) == [Pitch("B4"), Pitch("D#"), Pitch("F#")]
Test.@test simplifyMultipleEnharmonics([3, 8, 0]) == [Pitch("E-"), Pitch("A-"), Pitch("C")]
Test.@test simplifyMultipleEnharmonics([Pitch("G3"), Pitch("C-4"), Pitch("D4")]) == [Pitch("G3"), Pitch("B3"), Pitch("D4")]
Test.@test simplifyMultipleEnharmonics([Pitch("D--3"), Pitch("F-3"), Pitch("A--3")]) == [Pitch("D--3"), Pitch("F-3"), Pitch("A--3")]
Test.@test simplifyMultipleEnharmonics([Pitch("D--3"), Pitch("F-3"), Pitch("A--3")], keyContext=Key("C")) == [Pitch("C3"), Pitch("E3"), Pitch("G3")]
Test.@test simplifyMultipleEnharmonics([Pitch("B5")], keyContext=Key("D")) == [Pitch("B5")]

# -----------------------------------------------------------------------------


mutable struct AccidentalException <: Music21Exception
end


mutable struct PitchException <: Music21Exception
end

mutable struct  MicrotoneException <: Music21Exception
end
